...
if (!isCodex) try {
  ...
  // ponytail: transplanted as a project plugin, so statusLine may live in the
  // project's .claude settings, not just ~/.claude/settings.json. Check both.
  const candidatePaths = [settingsPath];
  if (process.env.CLAUDE_PROJECT_DIR) {
    candidatePaths.push(
      path.join(process.env.CLAUDE_PROJECT_DIR, '.claude', 'settings.local.json'),
      path.join(process.env.CLAUDE_PROJECT_DIR, '.claude', 'settings.json'),
    );
  }
  for (const p of candidatePaths) {
    try {
      if (fs.existsSync(p) && JSON.parse(fs.readFileSync(p, 'utf8')).statusLine) {
        hasStatusline = true;
        break;
      }
    } catch (e) { /* ignore unreadable/invalid settings */ }
  }
